    ...
    try:
        ...
        with stream_cloudwatch_logs(
            ...
            if worker_bootstrap.host_config and not HOST_CONFIGURATION_FEATURE:
                ...
                and worker_bootstrap.worker_info.host_configuration_marker
            ):
                _logger.info(
                    WorkerHostConfigurationLogEvent(
                        farm_id=config.farm_id,
            ...
            # Before the run looop starts, run the Host Configuration script.
                ...
                _logger.info(
                    WorkerHostConfigurationLogEvent(
                        ...
                        fleet_id=config.fleet_id,
                        worker_id=worker_id,
                        message="Worker Agent host Running Host Configuration.",
                ...
                host_config_runner = HostConfigurationScriptRunner(
                    ...
                    worker_id=worker_id,
                    session_directory=config.session_root_dir,
                ...
                if exit_code == 0:
                    _logger.info(
                        WorkerHostConfigurationLogEvent(
                            ...
                            fleet_id=config.fleet_id,
                            worker_id=worker_id,
                            message=f"Worker Agent host configuration successful. Exit code {exit_code}. Starting worker session loop.",
                            status=WorkerHostConfigurationStatus.RUNNING,
                            exit_code=exit_code,
                        )
                    )
                    # Persist host configuration has been performed.
                    worker_bootstrap.worker_info.host_configuration_marker = True
                    ...
                    _logger.critical(
                        WorkerHostConfigurationLogEvent(
                            ...
                            fleet_id=config.fleet_id,
                            worker_id=worker_id,
                            message=f"Worker Agent host configuration not successful. Exit code {exit_code}. Cannot run jobs. Exiting.",
