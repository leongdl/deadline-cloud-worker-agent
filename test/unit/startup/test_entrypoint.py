...
def test_host_config_already_run_before(
    ...
    # Given
    worker_info.host_configuration_marker = True
...
def test_fleet_host_config(
    ...
    if not has_host_config:
        mock_fleet_host_configuration_runner.assert_not_called()
        assert not worker_info.host_configuration_marker
    elif exit_code == 0:
        # Make sure we save the host configuration marker.
        assert worker_info.host_configuration_marker

        expected = (
            "Worker Agent host configuration successful. Exit code 0. Starting worker session loop."
        )
        ...
        expected = (
            "Worker Agent host configuration not successful. Exit code 1. Cannot run jobs. Exiting."
        )
